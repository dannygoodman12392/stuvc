...
router.get('/', (req, res) => {
  ...
  const notes = db.prepare(`
    ...
    WHERE f.is_deleted = 0 AND n.content LIKE ?
  ...
  const calls = db.prepare(`
    ...
    WHERE f.is_deleted = 0 AND (c.raw_transcript LIKE ? OR c.structured_summary LIKE ?)
  ...
  const memos = db.prepare(`
    ...
    WHERE f.is_deleted = 0 AND m.content LIKE ?
